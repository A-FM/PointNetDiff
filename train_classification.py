...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=32 * 2, help='batch size in training')
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader),
            ...
            if not args.use_cpu:
                points, target, yuanshi = points.cuda(), target.cuda(), points.cuda()
            yuanshi = points
            pred, trans_feat, gen = classifier(points)
            loss, dbl = criterion(pred, target.long(), trans_feat, gen, yuanshi)
            pred_choice = pred.data.max(1)[1]
            mean_loss.append(dbl.detach().cpu())
            correct = pred_choice.eq(target.long().data).cpu().sum()
            mean_correct.append(correct.item() / float(points.size()[0]))
            loss.backward()
            optimizer.step()
        ...
        log_string('db_mean_loss: %f' % np.mean(mean_loss))
        with torch.no_grad():
